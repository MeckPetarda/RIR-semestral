...
for t = 1 : deltaT : simulationTime
    ...
    % ===== TRANSFORM TO HEADING-ALIGNED FRAME =====
    % Rotation from NED to heading-aligned (body-fixed horizontal frame)
    % In heading-aligned frame: X_fwd = forward, Y_lat = lateral (right)
    cos_psi = cos(psi);
    sin_psi = sin(psi);
    
    % Position in heading-aligned frame
    x_fwd = X * cos_psi + Y * sin_psi;
    y_lat = -X * sin_psi + Y * cos_psi;
    
    % Velocity in heading-aligned frame
    v_fwd = vX * cos_psi + vY * sin_psi;
    v_lat = -vX * sin_psi + vY * cos_psi;
    
    % References in heading-aligned frame
    x_fwd_ref = x_ref * cos_psi + y_ref * sin_psi;
    y_lat_ref = -x_ref * sin_psi + y_ref * cos_psi;

    max_angle = 10*DegreeToRadian;

    % X REGULATOR (in heading-aligned frame - forward motion)
    % Position error in forward direction
    x_fwd_error = [x_fwd - x_fwd_ref; v_fwd];
    theta_des = max(min(-R_x * x_fwd_error, max_angle), -max_angle);

    % Y REGULATOR (in heading-aligned frame - lateral motion)
    % Position error in lateral direction
    y_lat_error = [y_lat - y_lat_ref; v_lat];
    phi_des = max(min(-R_y * y_lat_error, max_angle), -max_angle);
